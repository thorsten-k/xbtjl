...
public class BenDecoder
{
    ...
    public static Map<String,?> decode(byte[] data) throws IOException
    {
        return (new BenDecoder().decodeByteArray(data));
    }

    public static Map<String,?> decode(BufferedInputStream is) throws IOException
    {
        return (new BenDecoder().decodeStream(is));
    }

    public Map<String,?> decodeByteArray(byte[] data) throws IOException
    {
        return (decode(new ByteArrayInputStream(data)));
    }

    public Map<String,?> decodeStream(BufferedInputStream data) throws IOException
    {
    ...
    private Map<String,?> decode(ByteArrayInputStream data) throws IOException
    {
        Object res = decodeInputStream(data, 0);
    ...
    private Object decodeInputStream(InputStream bais, int nesting) throws IOException
    {
        ...
        case 'i': return new Long(getNumberFromStream(bais, 'e'));
    ...
    private void print(PrintWriter writer, Object obj, String indent, boolean skip_indent)
    {
        ...
        else if (obj instanceof byte[])
        {
            ...
            if (b.length == 20) {writer.println(use_indent + " { " + b + " }");}
            else if (b.length < 64){writer.println(new String(b));}
            else{writer.println("[byte array length " + b.length);}
